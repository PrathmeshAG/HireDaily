...
test("Meta true is the only value that becomes verified", async () => {
  const result = await checkInstagramUserFollow("igsid_1", {
    accessToken: "token",
    fetchImpl: async () => response({ is_user_follow_business: true, username: "test_user" }),
  });
  assert(result.status === "verified", "true -> verified");
  assert(result.username === "test_user", "Meta username is returned with follow verification");
});

test("Meta false remains not_verified", async () => {
  const result = await checkInstagramUserFollow("igsid_1", {
    accessToken: "token",
    fetchImpl: async () => response({ is_user_follow_business: false, username: "test_user" }),
  });
  assert(result.status === "not_verified", "false -> not_verified");
  assert(result.username === "test_user", "username is still available when not following");
});
