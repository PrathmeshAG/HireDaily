...
/**
 * Calculate Meta HMAC using the exact raw request bytes.
 */
...
/**
 * Meta-compatible escaped Unicode representation.
 */
...
export function verifyMetaWebhookSignature(
  ...
  // 2. Meta escaped-Unicode fallback
  if (/[\u0080-\uFFFF]/.test(body.toString("utf8"))) {
    const escapedBody =
      escapeUnicodeForMetaSignature(body);

    const escapedDigest =
      computeMetaDigest(escapedBody, secret);
 ...
 *
 * IMPORTANT:
 * This captures the exact bytes BEFORE JSON parsing.
 */
