...
export function getJobDateState(
  ...
  const closesToday = applyByTime !== null && !expired && new Date(applyByTime).toDateString() === new Date(now).toDateString();
  ...
  return {
    ...
    expired,
    closesToday,
    relativePosted: postedAt === null || postedAt > now ? null : formatRelativeTime(now - postedAt),
  };
}
